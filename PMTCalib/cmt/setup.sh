...
${CMTROOT}/mgr/cmt setup -sh -pack=PMTCalib -version=v0 -path=/junofs/users/yumiao/reconstruction/juno_wr/Calibration  -no_cleanup $* >${cmtPMTCalibtempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/mgr/cmt setup -sh -pack=PMTCalib -version=v0 -path=/junofs/users/yumiao/reconstruction/juno_wr/Calibration  -no_cleanup $* >${cmtPMTCalibtempfile}"
  cmtsetupstatus=2
