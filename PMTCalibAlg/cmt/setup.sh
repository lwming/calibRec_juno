...
${CMTROOT}/mgr/cmt setup -sh -pack=PMTCalibAlg -version=v0 -path=/junofs/users/zhangxt/20inch/rec/deconvolution  -no_cleanup $* >${cmtPMTCalibAlgtempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/mgr/cmt setup -sh -pack=PMTCalibAlg -version=v0 -path=/junofs/users/zhangxt/20inch/rec/deconvolution  -no_cleanup $* >${cmtPMTCalibAlgtempfile}"
  cmtsetupstatus=2
